...
extern volatile bool queue_send(queue_t queue, void *data);
extern volatile bool queue_receive(queue_t queue, void *data);

extern void register_interrupt(int pin, void (*isr)());
...
static bool shutdown_button_pressed = false;
static void button_press_interrupt_handler(void);

void initialize_control_loop(void) {
  torque_cmd_queue = queue_create(128, sizeof(torque_cmd_t));
  register_interrupt(15, button_press_interrupt_handler);
}
...
void torque_calculator_thread(void) {
  ...
  for(;;) {
    ...
    // ONLY ALLOW TORQUE IF WE HAVENT PRESSED THE BUTTON !!!!!!!!
    // PEOPLE COULD DIE
...
void button_press_interrupt_handler(void) { shutdown_button_pressed = true; }
